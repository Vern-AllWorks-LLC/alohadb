...
/*
 * LIRS (Low Inter-reference Recency Set) buffer eviction status values.
 * Used when the buffer_eviction_strategy GUC is set to 'lirs'.
 *
 ...
 * SIGMETRICS 2002.
 */
#define LIRS_STATUS_LIR				0	/* Low Inter-reference Recency block */
#define LIRS_STATUS_HIR_RESIDENT	1	/* High Inter-reference Recency, resident */
#define LIRS_STATUS_HIR_NONRESIDENT	2	/* High Inter-reference Recency, non-resident */

/*
 * Buffer eviction strategy enum values for the GUC.
 */
#define EVICTION_CLOCK	0
#define EVICTION_LIRS	1
...
/*
 * LIRS per-buffer metadata, stored in a separate shared memory array
 * to avoid increasing the size of BufferDesc beyond the 64-byte cache
 * line boundary.  Indexed by buf_id.  Protected by the strategy
 * spinlock (buffer_strategy_lock), not the buffer header lock.
 */
typedef struct LirsBufferInfo
{
	uint8		lirs_status;	/* LIRS_STATUS_LIR / HIR_RESIDENT / HIR_NONRESIDENT */
} LirsBufferInfo;

/* Pointer to shared array of per-buffer LIRS metadata */
extern PGDLLIMPORT LirsBufferInfo *LirsBufferInfoArray;
...
/* LIRS eviction strategy functions (freelist.c) */
extern void LirsAccessBuffer(BufferDesc *buf);

/* GUC variable for buffer eviction strategy */
extern PGDLLIMPORT int buffer_eviction_strategy;
...
/* GUC: use lock-free buffer mapping table (buf_table.c) */
extern PGDLLIMPORT bool enable_lockfree_bufmap;

/*
 * Lock-free Robin Hood hash table for buffer mapping.
 *
 * Each slot packs a 32-bit tag hash and a 21-bit buffer ID (plus flags) into
 * a single 64-bit atomic word.  Reads are naturally lock-free (atomic load),
 * inserts use CAS, and deletes use a tombstone marker.
 *
 * Slot encoding (64 bits):
 *   bits 63..32  -  tag hash (32 bits)
 *   bit  31      -  occupied flag
 *   bit  30      -  tombstone flag (deleted entry)
 *   bits 29..8   -  buf_id + 1 (22 bits, supports up to 4M buffers)
 *   bits 7..0    -  probe distance / displacement from ideal slot (8 bits)
 *
 * An empty slot has value LOCKFREE_BUF_EMPTY (0).
 */

#define LOCKFREE_BUF_EMPTY			UINT64CONST(0)
#define LOCKFREE_BUF_OCCUPIED		(UINT64CONST(1) << 31)
#define LOCKFREE_BUF_TOMBSTONE		(UINT64CONST(1) << 30)
#define LOCKFREE_BUF_HASH_SHIFT		32
#define LOCKFREE_BUF_HASH_MASK		UINT64CONST(0xFFFFFFFF00000000)
#define LOCKFREE_BUF_ID_SHIFT		8
#define LOCKFREE_BUF_ID_MASK		(UINT64CONST(0x3FFFFF) << 8)
#define LOCKFREE_BUF_DIST_MASK		UINT64CONST(0xFF)
#define LOCKFREE_BUF_MAX_DIST		255

/* Maximum number of buffers addressable in the lock-free scheme (22 bits) */
#define LOCKFREE_BUF_MAX_BUFFERS	((1 << 22) - 1)

/* Build a slot value from components */
static inline uint64
LockFreeBufSlotPack(uint32 tag_hash, int buf_id, uint8 dist)
{
	return ((uint64) tag_hash << LOCKFREE_BUF_HASH_SHIFT) |
		LOCKFREE_BUF_OCCUPIED |
		((uint64) (buf_id + 1) << LOCKFREE_BUF_ID_SHIFT) |
		(uint64) dist;
}

/* Extract tag hash from a slot value */
static inline uint32
LockFreeBufSlotGetHash(uint64 slot)
{
	return (uint32) (slot >> LOCKFREE_BUF_HASH_SHIFT);
}

/* Extract buf_id from a slot value (returns -1 for empty/tombstone) */
static inline int
LockFreeBufSlotGetBufId(uint64 slot)
{
	return (int) ((slot & LOCKFREE_BUF_ID_MASK) >> LOCKFREE_BUF_ID_SHIFT) - 1;
}

/* Extract probe distance from a slot value */
static inline uint8
LockFreeBufSlotGetDist(uint64 slot)
{
	return (uint8) (slot & LOCKFREE_BUF_DIST_MASK);
}

/* Check if a slot is occupied (and not a tombstone) */
static inline bool
LockFreeBufSlotIsOccupied(uint64 slot)
{
	return (slot & LOCKFREE_BUF_OCCUPIED) != 0 &&
		(slot & LOCKFREE_BUF_TOMBSTONE) == 0;
}

/* Check if a slot is empty (neither occupied nor tombstone) */
static inline bool
LockFreeBufSlotIsEmpty(uint64 slot)
{
	return slot == LOCKFREE_BUF_EMPTY;
}

/* Check if a slot is a tombstone */
static inline bool
LockFreeBufSlotIsTombstone(uint64 slot)
{
	return (slot & LOCKFREE_BUF_TOMBSTONE) != 0;
}

/*
 * LockFreeBufHash -- the shared lock-free hash table structure.
 *
 * Allocated in shared memory.  The slots array is sized to
 * next_power_of_2(requested_size * 2) for ~50% load factor.
 */
typedef struct LockFreeBufHash
{
	int			nslots;			/* number of slots (power of 2) */
	uint32		mask;			/* nslots - 1, for fast modular arithmetic */
	pg_atomic_uint64 slots[FLEXIBLE_ARRAY_MEMBER];
} LockFreeBufHash;
