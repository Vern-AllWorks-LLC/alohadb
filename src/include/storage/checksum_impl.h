...
/*
 * ----------
 * xxHash3-based page checksum implementation.
 *
 * This provides an alternative checksum algorithm using xxHash3 (64-bit),
 * which is faster than FNV-1a and has better collision resistance.
 *
 * The block number is mixed in as a seed to detect transposed pages,
 * analogous to the XOR in the FNV-1a path.
 *
 * The result is folded to 16 bits the same way as pg_checksum_page():
 *   (hash % 65535) + 1
 * so that zero checksums are avoided.
 * ----------
 */
#include "common/xxhash.h"

static uint16
pg_checksum_page_xxhash3_impl(char *page, BlockNumber blkno)
{
	uint16		save_checksum;
	uint64		hash;
	...
	/*
	 * Save pd_checksum and temporarily set it to zero, so that the
	 * checksum calculation isn't affected by the old checksum stored
	 * on the page.  Restore it after, because actually updating the
	 * checksum is NOT part of the API of this function.
	 */
	save_checksum = phdr->pd_checksum;
	phdr->pd_checksum = 0;

	hash = XXH3_64bits_withSeed(page, BLCKSZ, (uint64) blkno);
