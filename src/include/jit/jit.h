...
/*
 * Tiered JIT compilation tiers.
 *
 * Tier 0: Interpreted execution (ExecInterpExpr). No JIT overhead.
 * Tier 1: Fast JIT compile with LLVM -O0, no inlining.
 * Tier 2: Optimized JIT compile with LLVM -O3 + inlining.
 */
#define PGJIT_TIER_INTERPRET	0
#define PGJIT_TIER_FAST_COMPILE	1
#define PGJIT_TIER_OPT_COMPILE	2
...
typedef struct JitContext
{
	...
	/* Tiered JIT compilation state */
	int			jit_tier_count;		/* number of expression executions */
	int			jit_current_tier;	/* current compilation tier (PGJIT_TIER_*) */
...
extern PGDLLIMPORT int jit_tier1_threshold;
extern PGDLLIMPORT int jit_tier2_threshold;
...
extern bool jit_compile_expr_tiered(struct ExprState *state, int tier);
