 ...
 * mode and feed it data from the alohadb.bki library file.  After this
 ...
 * match what is in alohadb.conf.sample.
 *
...
set_null_conf(void)
{
	...
	path = psprintf("%s/alohadb.conf", pg_data);
...
setup_config(void)
{
	...
	/* ... and write out the finished alohadb.conf file */
	snprintf(path, sizeof(path), "%s/alohadb.conf", pg_data);
	...
	sprintf(path, "%s/alohadb.auto.conf", pg_data);
	...
	snprintf(path, sizeof(path), "%s/al_hba.conf", pg_data);
	...
	snprintf(path, sizeof(path), "%s/al_ident.conf", pg_data);
...
setup_bin_paths(const char *argv0)
{
	...
	if ((ret = find_other_exec(argv0, "alohadb", PG_BACKEND_VERSIONSTR,
							   backend_exec)) < 0)
	{
		...
		if (ret == -1)
			pg_fatal("program \"%s\" is needed by %s but was not found in the same directory as \"%s\"",
					 "alohadb", progname, full_path);
		else
			pg_fatal("program \"%s\" was found by \"%s\" but was not the same version as %s",
					 "alohadb", full_path, progname);
	}
...
{
	set_input(&bki_file, "alohadb.bki");
	set_input(&hba_file, "al_hba.conf.sample");
	set_input(&ident_file, "al_ident.conf.sample");
	set_input(&conf_file, "alohadb.conf.sample");
...
main(int argc, char *argv[])
{
	...
	if (authwarning)
	{
		...
		pg_log_warning_hint("You can change this by editing al_hba.conf or using the option -A, or "
	...
	if (!noinstructions)
	{
		...
		/* ... and tag on al_ctl instead */
		join_path_components(pg_ctl_path, pg_ctl_path, "al_ctl");

		/* Convert the path to use native separators */
		make_native_path(pg_ctl_path);

		/* path to al_ctl, properly quoted */
