...
/* Algorithm selection for --enable or --upgrade-algorithm */
static uint32 checksum_algorithm = DATA_CHECKSUM_VERSION_FNVA;
static bool upgrade_algorithm = false;
...
usage(void)
{
	...
	printf(_("      --algorithm=ALG      checksum algorithm: fnv (default) or xxhash3\n"));
	printf(_("      --upgrade-algorithm  upgrade existing checksum algorithm in-place\n"));
...
/*
 * Compute a page checksum using the given algorithm version.
 * This dispatches between FNV-1a (version 1) and xxHash3 (version 2).
 */
static uint16
compute_checksum(char *page, BlockNumber blkno, uint32 algo_version)
{
	if (algo_version == DATA_CHECKSUM_VERSION_XXHASH3)
		return pg_checksum_page_xxhash3_impl(page, blkno);
	else
...
scan_file(const char *fn, int segmentno)
{
	...
	flags = (mode == PG_MODE_ENABLE || upgrade_algorithm) ? O_RDWR : O_RDONLY;
	...
	for (blockno = 0;; blockno++)
	{
		...
		{
			/*
			 * When checking, use the algorithm recorded in the control file.
			 */
			csum = compute_checksum(buf.data,
									blockno + segmentno * RELSEG_SIZE,
		...
		else if (mode == PG_MODE_ENABLE)
		{
			...
			/*
			 * When enabling, use the selected algorithm (default: FNV-1a).
			 * When upgrading, recompute with the new target algorithm.
			 */
			csum = compute_checksum(buf.data,
									blockno + segmentno * RELSEG_SIZE,
									checksum_algorithm);
...
main(int argc, char *argv[])
{
	static struct option long_options[] = {
		...
		{"algorithm", required_argument, NULL, 2},
		{"upgrade-algorithm", no_argument, NULL, 3},
		{NULL, 0, NULL, 0}
	};
	...
	while ((c = getopt_long(argc, argv, "cdD:ef:NPv", long_options, &option_index)) != -1)
	{
		switch (c)
		{
			...
			case 2:
				/* --algorithm */
				if (strcmp(optarg, "fnv") == 0)
					checksum_algorithm = DATA_CHECKSUM_VERSION_FNVA;
				else if (strcmp(optarg, "xxhash3") == 0)
					checksum_algorithm = DATA_CHECKSUM_VERSION_XXHASH3;
				else
				{
					pg_log_error("unrecognized checksum algorithm \"%s\"", optarg);
					pg_log_error_hint("Supported algorithms are \"fnv\" and \"xxhash3\".");
					exit(1);
				}
				break;
			case 3:
				/* --upgrade-algorithm */
				upgrade_algorithm = true;
				break;
			default:
	...
	/* --algorithm only makes sense with --enable or --upgrade-algorithm */
	if (checksum_algorithm != DATA_CHECKSUM_VERSION_FNVA &&
		mode != PG_MODE_ENABLE && !upgrade_algorithm)
	{
		pg_log_error("option --algorithm can only be used with --enable or --upgrade-algorithm");
		pg_log_error_hint("Try \"%s --help\" for more information.", progname);
		exit(1);
	}

	/* --upgrade-algorithm requires --algorithm and implies --enable behavior */
	if (upgrade_algorithm && mode == PG_MODE_DISABLE)
	{
		pg_log_error("option --upgrade-algorithm cannot be used with --disable");
		pg_log_error_hint("Try \"%s --help\" for more information.", progname);
		exit(1);
	}
	...
	/*
	 * --upgrade-algorithm requires that checksums are already enabled so
	 * we can rewrite them with the new algorithm.
	 */
	if (upgrade_algorithm)
	{
		if (ControlFile->data_checksum_version == 0)
			pg_fatal("data checksums are not enabled in cluster, cannot upgrade algorithm");
		if (ControlFile->data_checksum_version == checksum_algorithm)
			pg_fatal("cluster is already using the requested checksum algorithm (version %u)",
					 checksum_algorithm);
		/*
		 * For algorithm upgrade, we operate in PG_MODE_ENABLE to rewrite
		 * all page checksums with the new algorithm.
		 */
		mode = PG_MODE_ENABLE;
	}
	...
	{
		ControlFile->data_checksum_version =
			(mode == PG_MODE_ENABLE) ? checksum_algorithm : 0;
		...
		if (mode == PG_MODE_ENABLE)
		{
			...
			if (upgrade_algorithm)
				printf(_("Checksums upgraded to algorithm \"%s\" in cluster\n"),
					   algo_name);
			else
				printf(_("Checksums enabled in cluster (algorithm: %s)\n"),
					   algo_name);
		}
		else
