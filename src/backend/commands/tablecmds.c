...
#include "commands/tablecmds_online.h"
...
/* GUC variable for non-blocking online DDL (Phase 6.2) */
bool		enable_online_ddl = true;
...
/* ----------------------------------------------------------------
 *		Online DDL helper functions (Phase 6.2)
 *
 *		These implement the non-blocking DDL optimizations controlled
 *		by the enable_online_ddl GUC.
 * ----------------------------------------------------------------
 */

/*
 * OnlineDDLCanUseFastSetNotNull
 *		Check whether the reduced-lock SET NOT NULL path is applicable.
 *
 * We only use the fast path for plain heap tables (not partitioned tables,
 * not tables with inheritance children, not foreign tables) when the
 * enable_online_ddl GUC is on.
 */
bool
OnlineDDLCanUseFastSetNotNull(Relation rel)
{
	if (!enable_online_ddl)
		return false;

	/* Only ordinary tables qualify */
	if (rel->rd_rel->relkind != RELKIND_RELATION)
		return false;

	/* Skip if the table has inheritance children */
 ...
 *		Check whether a column type change is binary-coercible and thus
 *		can skip the table rewrite entirely, only updating catalog metadata.
 */
bool
OnlineDDLIsBinaryCoercibleType(Oid oldtype, Oid newtype)
{
	if (!enable_online_ddl)
		return false;

	/* Same type is trivially coercible */
	if (oldtype == newtype)
		return true;

	return IsBinaryCoercible(oldtype, newtype);
}
	...
	foreach(lcmd, cmds)
	{
		...
		switch (cmd->subtype)
		{
			...
			case AT_AlterColumnType:	/* must rewrite heap */
				...
				/*
				 * When online DDL is enabled, binary-compatible type changes
				 * can be done with a weaker lock because they only update
				 * catalog metadata.  We optimistically take the weaker lock
				 * here; if at execution time the change turns out to require
				 * a rewrite, ATExecAlterColumnType will upgrade by closing
				 * and re-opening the relation with AccessExclusiveLock.
				 *
				 * Note: we cannot check IsBinaryCoercible here because we
				 * don't have the relation open yet and don't know the
				 * source type.  The actual eligibility check is deferred to
			...
			case AT_DisableTrigUser:
				...
				/*
				 * SET NOT NULL: when online DDL is enabled, we can validate
				 * existing rows under ShareUpdateExclusiveLock and only
				 * briefly take AccessExclusiveLock to flip the catalog flag.
				 */
			case AT_SetNotNull:
				if (enable_online_ddl)
					cmd_lockmode = ShareUpdateExclusiveLock;
				 ...
				 bool recurse, bool recursing, LOCKMODE lockmode)
{
	...
	/*
	 * Non-blocking online DDL path for SET NOT NULL (Phase 6.2).
	 *
	 * When the fast path is available, we perform the validation scan right
	 * here under ShareUpdateExclusiveLock (which allows concurrent reads and
	 * writes).  After verifying that no NULLs exist, we briefly acquire
	 * AccessExclusiveLock to flip the catalog flag.  This avoids blocking
	 * concurrent queries for the entire duration of the scan.
	 *
	 * For tables with children, partitioned tables, or when the GUC is off,
	 * we fall back to the original path that defers validation to Phase 3.
	 */
	if (!recursing && OnlineDDLCanUseFastSetNotNull(rel))
	{
		Oid			relid = RelationGetRelid(rel);
		Snapshot	snapshot;
		TableScanDesc scan;
		...
		/*
		 * Phase 1: Validate existing data under ShareUpdateExclusiveLock.
		 * Scan the entire table looking for NULLs in the target column.
		 */
		...
		/*
		 * Phase 2: Upgrade to AccessExclusiveLock to flip the catalog flag.
		 * This is a brief lock escalation -- the long scan has already
		 * completed under the weaker lock.
		 */
		LockRelationOid(relid, AccessExclusiveLock);

		/* Mark pg_attribute.attnotnull -- no further validation needed */
		set_attnotnull(wqueue, rel, attnum, true, false);

		ereport(DEBUG1,
				(errmsg_internal("online DDL: SET NOT NULL on \"%s\".\"%s\" validated under reduced lock",
								 RelationGetRelationName(rel), colName)));
	}
	else
	{
		/* Standard path: mark attnotnull and queue validation for Phase 3 */
					  ...
					  AlterTableUtilityContext *context)
{
	...
	{
		/*
		 * Online DDL fast path (Phase 6.2): if the type change is
		 * binary-coercible and no USING clause was given, we can skip the
		 * table rewrite entirely -- only catalog metadata needs updating.
		 * This allows the operation to proceed under ShareUpdateExclusiveLock
		 * without blocking concurrent reads and writes.
		 */
		if (!transform &&
			OnlineDDLIsBinaryCoercibleType(attTup->atttypid, targettype))
		{
			ereport(DEBUG1,
					(errmsg_internal("online DDL: column \"%s\" type change from %s to %s is binary-coercible, skipping rewrite",
									 colName,
									 format_type_be(attTup->atttypid),
									 format_type_be(targettype))));

			/*
			 * Still need to set up the coercion expression for
			 * ATExecAlterColumnType to work, but don't flag a rewrite.
			 */
			...
			/* Explicitly do NOT set tab->rewrite -- no rewrite needed */
		}
		else
		{
			/*
			 * Standard path: set up an expression to transform the old data
			 * value to the new type. If a USING option was given, use the
			 * expression as transformed by transformAlterTableStmt, else just
			 * take the old value and try to coerce it.  We do this first so
			 * that type incompatibility can be detected before we waste
			 * effort, and because we need the expression to be parsed against
			 * the original table row type.
			 */
			if (!transform)
			{
				transform = (Node *) makeVar(1, attnum,
			...
			/*
			 * Online DDL: if a rewrite is needed but we took a weaker lock
			 * because enable_online_ddl is on, we must upgrade to
			 * AccessExclusiveLock now.
			 */
			if ((tab->rewrite & AT_REWRITE_COLUMN_REWRITE) &&
				enable_online_ddl &&
				lockmode < AccessExclusiveLock)
			{
				...
				ereport(DEBUG1,
						(errmsg_internal("online DDL: column \"%s\" type change requires rewrite, upgraded to AccessExclusiveLock",
										 colName)));
			}
		}
	}
					  ...
					  AlterTableCmd *cmd, LOCKMODE lockmode)
{
	...
	/*
	 * Online DDL: if no rewrite is needed and we're still holding a weaker
	 * lock, upgrade briefly for the catalog update.  This ensures safety
	 * while keeping the expensive rewrite/scan phase (which didn't happen)
	 * out of the AccessExclusiveLock window.
	 */
	if (!tab->rewrite && enable_online_ddl && lockmode < AccessExclusiveLock)
		LockRelationOid(RelationGetRelid(rel), AccessExclusiveLock);
