...
PinBuffer(BufferDesc *buf, BufferAccessStrategy strategy)
{
	...
	/*
	 * If LIRS eviction strategy is active and this is the first pin (we just
	 * created a new refcount entry), update the LIRS state for this buffer.
	 * We call this only for initial pins to avoid updating LIRS metadata on
	 * every re-pin of an already-pinned buffer.
	 */
	if (ref->refcount == 1)
		LirsAccessBuffer(buf);
