...
PageIsVerified(PageData *page, BlockNumber blkno, int flags, bool *checksum_failure_p)
{
	...
	if (!PageIsNew(page))
	{
		...
		{
			uint32	cksumver = GetDataChecksumsVersion();

			if (cksumver == DATA_CHECKSUM_VERSION_XXHASH3)
				checksum = pg_checksum_page_xxhash3(page, blkno);
			else
...
PageSetChecksumCopy(Page page, BlockNumber blkno)
{
...

	{
		...
		if (cksumver == DATA_CHECKSUM_VERSION_XXHASH3)
			((PageHeader) pageCopy)->pd_checksum = pg_checksum_page_xxhash3(pageCopy, blkno);
		else
...
PageSetChecksumInplace(Page page, BlockNumber blkno)
{
	...
	cksumver = GetDataChecksumsVersion();

	if (cksumver == DATA_CHECKSUM_VERSION_XXHASH3)
		((PageHeader) page)->pd_checksum = pg_checksum_page_xxhash3(page, blkno);
	else
