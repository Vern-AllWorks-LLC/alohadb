...
PostmasterMain(int argc, char *argv[])
{
	...
	/*
	 * Apply auto-tuning configuration profile if one is set.
	 * This must happen after config files are read (so we know the profile
	 * name) but before GUC validation checks (so profile-set values like
	 * max_connections are in effect for the checks below).
	 */
	if (guc_profile != NULL && guc_profile[0] != '\0')
		ApplyProfile(guc_profile);
