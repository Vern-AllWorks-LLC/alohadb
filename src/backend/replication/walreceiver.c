		...
		if (walrcv_startstreaming(wrconn, &options))
		{
			...
			for (;;)
			{
				...
				if (len != 0)
				{
					...
					/*
					 * If Raft mode is enabled, drive the Raft state
					 * machine on the follower side.  This checks the
					 * election timeout and processes any pending state
					 * transitions.
					 */
...
XLogWalRcvProcessMsg(unsigned char type, char *buf, Size len, TimeLineID tli)
{
	...
	switch (type)
	{
		...
		/*
		 * Raft protocol messages.  When running in Raft mode, the leader
		 * (walsender) may send AppendEntries, RequestVote, or Heartbeat
		 * messages interleaved with normal WAL data.
		 */
		...
		case RAFT_MSG_HEARTBEAT:
			...
			{
				/*
				 * Reconstruct the full Raft message with the type byte
				 * included, then hand it off to the dispatcher.
				 */
				char	   *raftbuf = palloc(len + 1);

				raftbuf[0] = (char) type;
				if (len > 0)
					memcpy(raftbuf + 1, buf, len);

				RaftProcessMessage(raftbuf, len + 1);
				pfree(raftbuf);
			}
			else
			{
				ereport(ERROR,
						(errcode(ERRCODE_PROTOCOL_VIOLATION),
						 errmsg_internal("received Raft message type %d but replication_mode is not 'raft'",
										 type)));
			}
			break;
