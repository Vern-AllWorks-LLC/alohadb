...
ProcessStandbyMessage(void)
{
	...
	switch (msgtype)
	{
		...
		/*
		 * Raft protocol messages.  When replication_mode = 'raft', standby
		 * nodes send Raft RPCs wrapped in CopyData messages.  The first
		 * byte of the inner payload identifies the Raft message type.
		 */
		...
		case RAFT_MSG_HEARTBEAT:
			...
			{
				/*
				 * Reconstruct the full message: prepend the type byte
				 * that we already consumed, then hand it to the Raft
				 * message dispatcher.
				 */
				StringInfoData raftmsg;

				initStringInfo(&raftmsg);
				appendStringInfoChar(&raftmsg, msgtype);
				appendBinaryStringInfo(&raftmsg,
									   pq_getmsgbytes(&reply_message,
													   reply_message.len - reply_message.cursor),
									   reply_message.len - reply_message.cursor);
				RaftProcessMessage(raftmsg.data, raftmsg.len);
				pfree(raftmsg.data);
			}
			else
			{
				ereport(COMMERROR,
						(errcode(ERRCODE_PROTOCOL_VIOLATION),
						 errmsg("received Raft message but replication_mode is not 'raft'")));
				proc_exit(0);
			}
			break;
...
WalSndLoop(WalSndSendDataCallback send_data)
{
	...
	for (;;)
	{
		...
		/*
		 * If Raft mode is enabled, drive the Raft state machine.
		 * This handles heartbeats (leader), election timeouts
		 * (follower/candidate), and commit index advancement.
		 */
		if (replication_mode == REPLICATION_MODE_RAFT)
			RaftTick();
