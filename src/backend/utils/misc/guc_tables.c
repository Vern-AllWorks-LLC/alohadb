...
static const struct config_enum_entry buffer_eviction_strategy_options[] = {
	{"clock", EVICTION_CLOCK, false},
	{"lirs", EVICTION_LIRS, false},
	{NULL, 0, false}
};

static const struct config_enum_entry replication_mode_options[] = {
	{"streaming", REPLICATION_MODE_STREAMING, false},
	{"raft", REPLICATION_MODE_RAFT, false},
	{NULL, 0, false}
};
...
/* support for legacy compile-time setting */
	...
	{
		{"enable_lockfree_bufmap", PGC_POSTMASTER, RESOURCES_MEM,
			gettext_noop("Uses a lock-free hash table for shared buffer mapping."),
			gettext_noop("When enabled, buffer tag lookups bypass BufMappingLock "
						 "using atomic operations, improving scalability on "
						 "high-core-count systems."),
		},
		&enable_lockfree_bufmap,
		false,
		NULL, NULL, NULL
	},

	{
		{"enable_online_ddl", PGC_USERSET, CLIENT_CONN_STATEMENT,
			gettext_noop("Enables non-blocking online DDL optimizations for ALTER TABLE."),
			gettext_noop("When enabled, certain ALTER TABLE operations such as SET NOT NULL "
						 "and compatible ALTER COLUMN TYPE use reduced locking to allow "
...
struct config_int ConfigureNamesInt[] =
{
	...
	{
		{"jit_tier1_threshold", PGC_USERSET, QUERY_TUNING_OTHER,
			gettext_noop("Number of expression executions before tier-1 JIT compilation."),
			gettext_noop("After this many executions, the expression is compiled "
						 "with LLVM at -O0 (fast compile, low latency). "
						 "Set to 0 to disable tiered JIT and use the traditional "
						 "immediate compilation behavior."),
			GUC_EXPLAIN
		},
		&jit_tier1_threshold,
		5, 0, INT_MAX,
	...
	{
		{"jit_tier2_threshold", PGC_USERSET, QUERY_TUNING_OTHER,
			gettext_noop("Number of expression executions before tier-2 JIT recompilation."),
			gettext_noop("After this many executions, the expression is recompiled "
						 "with LLVM at -O3 with inlining (optimized compile, max throughput). "
						 "Set to 0 to disable tier-2 recompilation."),
			GUC_EXPLAIN
		},
		&jit_tier2_threshold,
		50, 0, INT_MAX,
	...
	{
		{"wal_group_commit_delay", PGC_SUSET, WAL_SETTINGS,
			gettext_noop("Sets the adaptive group commit delay in microseconds."),
			gettext_noop("When WAL flush latency exceeds 1ms, the group commit "
						 "leader delays by this amount to batch more flush requests. "
						 "Set to 0 to disable adaptive batching delay.")
		},
		&wal_group_commit_delay,
		10, 0, 100,
		NULL, NULL, NULL
	},
	...
	{
		{"ivfflat.probes", PGC_USERSET, CLIENT_CONN_OTHER,
			gettext_noop("Sets the number of probes for IVFFlat index scans."),
			NULL,
		},
		&ivfflat_probes,
		1, 1, 10000,
	...
	{
		{"raft_election_timeout_ms", PGC_SIGHUP, REPLICATION_SENDING,
			gettext_noop("Sets the base election timeout for Raft consensus in milliseconds."),
			gettext_noop("The actual timeout is randomised between this value and twice this value. "
						 "Only effective when replication_mode = 'raft'."),
			GUC_UNIT_MS
		},
	...
	{
		{"raft_heartbeat_interval_ms", PGC_SIGHUP, REPLICATION_SENDING,
			gettext_noop("Sets the heartbeat interval for the Raft leader in milliseconds."),
			gettext_noop("The leader sends periodic heartbeats to prevent followers from "
						 "starting new elections. Should be much less than raft_election_timeout_ms. "
						 "Only effective when replication_mode = 'raft'."),
			GUC_UNIT_MS
		},
...
struct config_string ConfigureNamesString[] =
{
	...
	{
		{"profile", PGC_SIGHUP, CUSTOM_OPTIONS,
			gettext_noop("Selects an auto-tuning configuration profile."),
			gettext_noop("Valid values are: oltp, analytics, ai_ml, hybrid. "
						 "An empty string disables profile-based tuning.")
		},
		&guc_profile,
		"",
		check_profile, assign_profile, NULL
	},

	{
		{"raft_members", PGC_SIGHUP, REPLICATION_SENDING,
			gettext_noop("Sets the list of Raft cluster members."),
			gettext_noop("A comma-separated list of host:port pairs identifying "
						 "all nodes in the Raft cluster. Example: "
						 "\"node1:5432,node2:5432,node3:5432\". "
						 "Only effective when replication_mode = 'raft'."),
			GUC_LIST_INPUT
		},
		&raft_members,
		"",
		RaftCheckMembersGUC, RaftAssignMembersGUC, NULL
	},
...
struct config_enum ConfigureNamesEnum[] =
{
	...
	{
		{"buffer_eviction_strategy", PGC_POSTMASTER, RESOURCES_MEM,
			gettext_noop("Selects the buffer pool eviction algorithm."),
			gettext_noop("\"clock\" uses the traditional clock-sweep algorithm. "
						 "\"lirs\" uses the LIRS (Low Inter-reference Recency Set) "
						 "algorithm which can provide better hit rates for "
						 "workloads with varying access patterns.")
		},
		&buffer_eviction_strategy,
		EVICTION_CLOCK, buffer_eviction_strategy_options,
		NULL, NULL, NULL
	},
	...
	{
		{"replication_mode", PGC_POSTMASTER, REPLICATION_SENDING,
			gettext_noop("Selects the replication protocol."),
			gettext_noop("\"streaming\" uses traditional streaming replication. "
						 "\"raft\" uses Raft consensus on top of WAL replication "
						 "for stronger consistency guarantees.")
		},
		&replication_mode,
		REPLICATION_MODE_STREAMING, replication_mode_options,
		NULL, NULL, NULL
	},
