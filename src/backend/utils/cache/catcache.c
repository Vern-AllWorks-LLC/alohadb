...
/* ----------------------------------------------------------------
 *		Epoch-based reclamation (RCU pattern) for catalog cache
 *
 * The catalog cache is per-backend, but invalidation messages can arrive
 * at any point during a catalog lookup (e.g., during TOAST table access
 * or table_open() calls).  The epoch-based scheme allows readers to
 * traverse cache entries without risk of use-after-free:
 *
 *   - On each read-side critical section entry, we increment a nesting
 *     counter (catcache_rcu_reader_depth) and capture the current global
 *     epoch.
 *
 *   - On exit, we decrement the nesting counter.  When it reaches zero,
 *     we advance the reader's epoch and attempt to drain the pending
 *     reclamation list.
 *
 *   - Invalidation (the "writer") does not free entries immediately.
 *     Instead it unlinks them from the hash chain and appends them to
 *     a deferred-free list tagged with the current epoch.
 *
 *   - Deferred entries are actually freed only once the reader epoch has
 *     advanced past the epoch at which the entry was retired.
 *
 * Because PostgreSQL backends are single-threaded, the "concurrency"
 * here is between the main execution path (reader) and invalidation
 * callbacks that can fire re-entrantly.
 * ----------------------------------------------------------------
 */

/*
 * Maximum number of entries on the deferred reclamation list before we
 * force a drain attempt.
 */
#define CATCACHE_RCU_DRAIN_THRESHOLD	64

/*
 * A deferred-free list entry.  We keep a singly-linked list of these.
 */
typedef struct CatCacheRCUPending
{
	struct CatCacheRCUPending *next;
	uint64		retire_epoch;	/* epoch at which entry was retired */
	CatCache   *cache;			/* owning cache (for stats/keys cleanup) */
	CatCTup    *ct;				/* cache tuple entry, or NULL */
	CatCList   *cl;				/* cache list entry, or NULL */
} CatCacheRCUPending;

/* Global epoch, monotonically increasing */
static uint64 catcache_rcu_global_epoch = 0;

/* Reader's observed epoch; entries retired before this can be freed */
static uint64 catcache_rcu_reader_epoch = 0;

/* Nesting depth of read-side critical sections */
static int	catcache_rcu_reader_depth = 0;

/* Deferred reclamation list (singly linked) */
static CatCacheRCUPending *catcache_rcu_pending_head = NULL;
static int	catcache_rcu_pending_count = 0;

/*
 * CatCacheRCUReaderEnter
 *		Enter a read-side critical section.  Safe to nest.
 */
static inline void
CatCacheRCUReaderEnter(void)
{
	if (catcache_rcu_reader_depth == 0)
		catcache_rcu_reader_epoch = catcache_rcu_global_epoch;
	catcache_rcu_reader_depth++;
}

/*
 * Forward declaration for the drain function.
 */
static void CatCacheRCUDrain(void);
static void CatCacheFreeKeys(TupleDesc tupdesc, int nkeys, int *attnos,
							 Datum *keys);

/*
 * CatCacheRCUReaderExit
 *		Leave a read-side critical section.  When the outermost section
 *		exits, advance the reader epoch and try to drain deferred frees.
 */
static inline void
CatCacheRCUReaderExit(void)
{
	Assert(catcache_rcu_reader_depth > 0);
	catcache_rcu_reader_depth--;
	...
	if (catcache_rcu_reader_depth == 0)
	{
		...
		if (catcache_rcu_pending_count > 0)
			CatCacheRCUDrain();
	}
}

/*
 * CatCacheRCUDeferFree
 *		Instead of freeing a CatCTup immediately, append it to the
 *		deferred list for later reclamation.
 */
static void
CatCacheRCUDeferFreeCTup(CatCache *cache, CatCTup *ct)
{
	CatCacheRCUPending *entry;

	/*
	 * Advance the global epoch so that any reader currently active will
	 * hold an epoch strictly less than the retire_epoch.
	 */
	catcache_rcu_global_epoch++;

	entry = (CatCacheRCUPending *) palloc(sizeof(CatCacheRCUPending));
	entry->retire_epoch = catcache_rcu_global_epoch;
	entry->cache = cache;
	entry->ct = ct;
	entry->cl = NULL;
	entry->next = catcache_rcu_pending_head;
	catcache_rcu_pending_head = entry;
	catcache_rcu_pending_count++;
}

/*
 * CatCacheRCUDeferFreeCList
 *		Deferred-free for a CatCList.
 */
static void
CatCacheRCUDeferFreeCList(CatCache *cache, CatCList *cl)
{
	CatCacheRCUPending *entry;
...
/*
 * CatCacheRCUDrain
 *		Free all deferred entries whose retire epoch is older than the
 *		current reader epoch.  Called when the reader exits its outermost
 *		critical section.
 */
static void
CatCacheRCUDrain(void)
{
	CatCacheRCUPending *prev = NULL;
	CatCacheRCUPending *cur = catcache_rcu_pending_head;

	while (cur != NULL)
	{
		CatCacheRCUPending *next = cur->next;

		/*
		 * Safe to free if no reader could possibly be referencing it.
		 * Since we are single-threaded, this means the reader epoch must
		 * have advanced past the retire epoch, OR we are outside any
		 * read-side critical section (depth == 0).
		 */
		if (catcache_rcu_reader_depth == 0 ||
			catcache_rcu_reader_epoch >= cur->retire_epoch)
		{
			/* Unlink from list */
			if (prev == NULL)
				catcache_rcu_pending_head = next;
			else
				prev->next = next;
			catcache_rcu_pending_count--;
			...
			{
				CatCTup    *ct = cur->ct;
				CatCache   *cache = cur->cache;

				/*
				 * Free keys for negative entries (positive entries' keys
				 * point into the tuple which is allocated together).
				 */
				if (ct->negative)
...
CatCacheRemoveCTup(CatCache *cache, CatCTup *ct)
{
	...
	/*
	 * If a reader is currently traversing a hash bucket (RCU read-side
	 * critical section is active), defer the actual pfree so the reader
	 * does not encounter freed memory.
	 */
	if (catcache_rcu_reader_depth > 0)
	{
		CatCacheRCUDeferFreeCTup(cache, ct);
		return;
	}
...
CatCacheRemoveCList(CatCache *cache, CatCList *cl)
{
	...
	/*
	 * Defer the actual free if a reader is active (RCU pattern).
	 */
	if (catcache_rcu_reader_depth > 0)
	{
		CatCacheRCUDeferFreeCList(cache, cl);
		return;
	}
					   ...
					   Datum v4)
{
	...
	/*
	 * Enter RCU read-side critical section.  While we are inside this
	 * section, any CatCacheInvalidate call that fires (e.g., from
	 * AcceptInvalidationMessages during re-entrant catalog access) will
	 * defer freeing cache entries rather than freeing them immediately.
	 * This prevents use-after-free when we are iterating the hash bucket.
	 */
	CatCacheRCUReaderEnter();
				   ...
				   Datum v3)
{
	...
	/*
	 * Enter RCU read-side critical section to protect against re-entrant
	 * invalidation freeing list entries while we traverse the bucket.
	 */
	CatCacheRCUReaderEnter();
