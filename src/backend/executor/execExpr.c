 ...
 */
static Datum ExecTieredJitDispatch(ExprState *state, ExprContext *econtext, bool *isNull);

/*
 * Per-expression state for the tiered JIT dispatch wrapper.
 * Tracks the execution count and the original interpreter evalfunc so
 * we can delegate to it while the expression is still in the interpreted tier.
 */
typedef struct TieredJitDispatchState
{
	ExprStateEvalFunc interp_evalfunc;	/* saved interpreter evalfunc */
	void	   *interp_evalfunc_private; /* saved interpreter evalfunc_private */
	int			exec_count;				/* per-expression execution counter */
	int			current_tier;			/* PGJIT_TIER_* */
	bool		first_call;				/* true until first interpreted call */
} TieredJitDispatchState;
...
ExecReadyExpr(ExprState *state)
{
	...
	/*
	 * If tiered JIT compilation is enabled and this expression is eligible
	 * for JIT (has a parent PlanState with PGJIT_PERFORM and PGJIT_EXPR),
	 * wrap the interpreted evalfunc with our tiered dispatch function.
	 * The dispatch function counts executions and triggers JIT compilation
	 * when thresholds are reached.
	 */
	if (jit_enabled && jit_tier1_threshold > 0 &&
		state->parent &&
		(state->parent->state->es_jit_flags & PGJIT_PERFORM) &&
		(state->parent->state->es_jit_flags & PGJIT_EXPR))
	{
		TieredJitDispatchState *tstate;

		tstate = (TieredJitDispatchState *) palloc0(sizeof(TieredJitDispatchState));
		tstate->interp_evalfunc = state->evalfunc;
		tstate->interp_evalfunc_private = state->evalfunc_private;
		tstate->exec_count = 0;
		tstate->current_tier = PGJIT_TIER_INTERPRET;
		tstate->first_call = true;

		state->evalfunc = ExecTieredJitDispatch;
		state->evalfunc_private = tstate;
	}
}

/*
 * ExecTieredJitDispatch - tiered JIT compilation dispatch wrapper.
 *
 * This function wraps expression evaluation with execution counting.
 * When the execution count reaches jit_tier1_threshold, the expression
 * is compiled with LLVM at -O0 (fast compile).  When it reaches
 * jit_tier2_threshold, the expression is recompiled with -O3 + inlining
 * for maximum throughput.
 *
 * Between tier promotions, the function delegates to the saved interpreter
 * evalfunc (initially ExecInterpExprStillValid, then the resolved fast-path
 * interpreter function).
 */
static Datum
ExecTieredJitDispatch(ExprState *state, ExprContext *econtext, bool *isNull)
{
	TieredJitDispatchState *tstate = (TieredJitDispatchState *) state->evalfunc_private;
	int			exec_count;

	exec_count = ++tstate->exec_count;

	/*
	 * Check if we should promote to tier 2 (optimized compile).
	 * This triggers recompilation with -O3 + inlining.
	 */
	if (tstate->current_tier == PGJIT_TIER_FAST_COMPILE &&
		jit_tier2_threshold > 0 &&
		exec_count >= jit_tier2_threshold)
	{
		if (jit_compile_expr_tiered(state, PGJIT_TIER_OPT_COMPILE))
		{
			tstate->current_tier = PGJIT_TIER_OPT_COMPILE;
			elog(DEBUG1, "tiered JIT: expression promoted to tier 2 after %d executions",
				 exec_count);

			/*
			 * The JIT provider has replaced state->evalfunc with the compiled
			 * expression runner (ExecRunCompiledExpr).  From now on, calls go
			 * directly to the tier-2 JITed code.  The dispatch wrapper is no
			 * longer involved -- state->evalfunc and state->evalfunc_private
			 * have been set by the provider.
			 */
	...
	/*
	 * Check if we should promote to tier 1 (fast compile).
	 */
	if (tstate->current_tier == PGJIT_TIER_INTERPRET &&
		exec_count >= jit_tier1_threshold)
	{
		if (jit_compile_expr_tiered(state, PGJIT_TIER_FAST_COMPILE))
		{
			Datum		result;

			tstate->current_tier = PGJIT_TIER_FAST_COMPILE;
			elog(DEBUG1, "tiered JIT: expression promoted to tier 1 after %d executions",
				 exec_count);

			/*
			 * The JIT provider has replaced state->evalfunc with the compiled
			 * expression runner.  Save the new evalfunc and its private state
			 * so we can delegate to it, then reinstall ourselves for tier-2
			 * tracking.
			 *
			 * ExecRunCompiledExpr uses state->evalfunc_private to find its
			 * CompiledExprState, so we must leave state->evalfunc_private set
			 * to the provider's value during the call, then restore ours.
			 */
			tstate->interp_evalfunc = state->evalfunc;
			tstate->interp_evalfunc_private = state->evalfunc_private;

			/* Call the tier-1 compiled function with its own private state */
			result = state->evalfunc(state, econtext, isNull);

			/*
			 * After the first call, ExecRunCompiledExpr may have replaced
			 * state->evalfunc with the direct JITed function pointer.
			 * Capture that and reinstall our wrapper.
			 */
			tstate->interp_evalfunc = state->evalfunc;
			tstate->interp_evalfunc_private = state->evalfunc_private;
			state->evalfunc = ExecTieredJitDispatch;
	...
	/*
	 * Not yet at a promotion threshold, or promotion failed.
	 * Delegate to the interpreter.
	 *
	 * On the first call, we use the saved evalfunc (ExecInterpExprStillValid)
	 * which performs validity checking and then resolves to the actual
	 * interpreter function.  We need to handle the fact that
	 * ExecInterpExprStillValid replaces state->evalfunc -- we must
	 * capture that and reinstall our wrapper.
	 */
	if (tstate->first_call)
	{
		Datum		result;

		tstate->first_call = false;

		/*
		 * Temporarily install the interpreter's evalfunc so
		 * ExecInterpExprStillValid can do its thing (it sets
		 * state->evalfunc = state->evalfunc_private).
		 */
		state->evalfunc = tstate->interp_evalfunc;
		state->evalfunc_private = tstate->interp_evalfunc_private;

		result = state->evalfunc(state, econtext, isNull);

		/*
		 * ExecInterpExprStillValid has now replaced state->evalfunc
		 * with the actual interpreter function.  Save it and reinstall
		 * our wrapper.
		 */
		tstate->interp_evalfunc = state->evalfunc;
		tstate->interp_evalfunc_private = state->evalfunc_private;
	...
	/*
	 * Subsequent calls at the current tier: delegate to the saved evalfunc.
	 * We must temporarily restore evalfunc_private in case the delegate
	 * function reads it (e.g., ExecRunCompiledExpr reads its
	 * CompiledExprState from evalfunc_private).
	 */
	{
		Datum		result;

		state->evalfunc_private = tstate->interp_evalfunc_private;
		result = tstate->interp_evalfunc(state, econtext, isNull);

		/*
		 * The delegate may have replaced evalfunc (e.g., ExecRunCompiledExpr
		 * replaces itself with the direct JITed function on first call).
		 * Capture updates and reinstall our wrapper.
		 */
		if (state->evalfunc != ExecTieredJitDispatch)
		{
			tstate->interp_evalfunc = state->evalfunc;
			tstate->interp_evalfunc_private = state->evalfunc_private;
			state->evalfunc = ExecTieredJitDispatch;
