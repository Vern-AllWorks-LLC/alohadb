...
/* Flag to ensure we only register the vectorized-scan hook once */
static bool vectorscan_hook_registered = false;
...
ExecInitNode(Plan *node, EState *estate, int eflags)
{
	...
	/*
	 * Register the vectorized-scan CustomScan provider on first call.
	 * This hooks into set_rel_pathlist_hook so that subsequent planning
	 * can inject vectorized scan paths.  We do it here rather than in
	 * a shared_preload_library because the vectorized engine is built
	 * into the core executor.
	 */
	if (!vectorscan_hook_registered)
	{
		VectorScan_RegisterHook();
		vectorscan_hook_registered = true;
	}
