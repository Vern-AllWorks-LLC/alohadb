...
int			wal_group_commit_delay = 10;	/* adaptive group commit delay in us */
...
/*
 * WAL Group Commit with Adaptive Batching (Phase 5.2)
 *
 * When multiple backends are trying to flush WAL simultaneously, we batch
 * their requests so that a single leader performs one fsync() covering all
 * queued LSNs.  This significantly reduces the number of fsyncs under high
 * concurrency.
 *
 * Adaptive batching: we measure the flush latency of each fsync.  If the
 * latency is below 1ms, the system is under low load and we skip the
 * batching delay.  If the latency exceeds 1ms, we briefly delay (up to
 * wal_group_commit_delay microseconds) to collect more waiters before
 * issuing the next fsync.
 */
#define GROUP_COMMIT_MAX_WAITERS	128

typedef struct GroupCommitWaiter
{
	XLogRecPtr	flush_lsn;		/* LSN this backend wants flushed */
	int			pgprocno;		/* index into ProcGlobal->allProcs for latch */
} GroupCommitWaiter;

typedef struct GroupCommitState
{
	pg_atomic_uint32 flush_in_progress; /* 1 if a leader is flushing */
	slock_t		queue_lock;		/* protects queue manipulation */
	int			num_waiters;	/* number of queued waiters */
	GroupCommitWaiter waiters[GROUP_COMMIT_MAX_WAITERS];
	int64		last_flush_latency_ns;	/* latency of last fsync in ns */
} GroupCommitState;

static GroupCommitState *groupCommitState = NULL;
 ...
 *
 * WAL Group Commit with Adaptive Batching (Phase 5.2):
 * When a flush is already in progress, instead of immediately contending for
 * the WALWriteLock, we queue behind the current leader.  The leader flushes
 * all queued LSNs with a single fsync(), reducing total fsync count.
 *
 * Adaptive batching measures flush latency.  Under low load (latency < 1ms),
 * we skip the batching delay.  Under high load (latency >= 1ms), we briefly
 * delay up to wal_group_commit_delay microseconds to collect more waiters.
 */
void
XLogFlush(XLogRecPtr record)
{
	...
	/*
	 * WAL Group Commit: attempt to join an in-progress flush as a follower.
	 *
	 * If flush_in_progress is set, another backend is currently performing
	 * the fsync.  We add our LSN to the queue and wait for the leader to
	 * complete a flush that covers our LSN.  This avoids redundant fsyncs.
	 *
	 * We only attempt group commit when we have a valid MyProc (i.e., we are
	 * a normal backend, not in bootstrap or single-user mode) and group
	 * commit state is initialized.
	 */
	if (groupCommitState != NULL && MyProc != NULL &&
		pg_atomic_read_u32(&groupCommitState->flush_in_progress) != 0)
	{
		bool	queued = false;

		SpinLockAcquire(&groupCommitState->queue_lock);
		if (pg_atomic_read_u32(&groupCommitState->flush_in_progress) != 0 &&
			groupCommitState->num_waiters < GROUP_COMMIT_MAX_WAITERS)
		{
			int		idx = groupCommitState->num_waiters++;

			groupCommitState->waiters[idx].flush_lsn = record;
			groupCommitState->waiters[idx].pgprocno = MyProcNumber;
			queued = true;
		}
		SpinLockRelease(&groupCommitState->queue_lock);

		if (queued)
		{
			/*
			 * Wait for the leader to flush our LSN.  We use a latch wait
			 * with a timeout to handle edge cases where the leader might
			 * have already finished before we started waiting.
			 */
			for (;;)
			{
				RefreshXLogWriteResult(LogwrtResult);
				...
				(void) WaitLatch(&MyProc->procLatch,
								 ...
								 1L,	/* 1ms timeout for recheck */
								 WAIT_EVENT_WAL_GROUP_COMMIT);
		...
		/* If queue was full, fall through to normal flush path */
	}

	START_CRIT_SECTION();

	/*
	 * Mark ourselves as the group commit leader.  This signals to other
	 * backends that they can queue behind us instead of contending for the
	 * WALWriteLock independently.
	 */
	if (groupCommitState != NULL)
		pg_atomic_write_u32(&groupCommitState->flush_in_progress, 1);
	...
	/*
	 * Collect LSNs from any backends that have queued behind us for group
	 * commit.  We advance WriteRqstPtr to cover the maximum queued LSN so
	 * that our single fsync() satisfies all waiters.
	 */
	if (groupCommitState != NULL)
	{
		SpinLockAcquire(&groupCommitState->queue_lock);
		for (int i = 0; i < groupCommitState->num_waiters; i++)
		{
			if (groupCommitState->waiters[i].flush_lsn > WriteRqstPtr)
				WriteRqstPtr = groupCommitState->waiters[i].flush_lsn;
		}
		SpinLockRelease(&groupCommitState->queue_lock);
	}
	...
	for (;;)
	{
		...
		/*
		 * Adaptive group commit delay (Phase 5.2): if the last flush took
		 * more than 1ms (indicating the system is under significant I/O
		 * load), and wal_group_commit_delay is configured, briefly sleep to
		 * allow more backends to queue their flush requests.  Under low load
		 * (flush < 1ms), we skip this delay entirely to minimize latency.
		 */
		if (groupCommitState != NULL &&
			wal_group_commit_delay > 0 && enableFsync &&
			groupCommitState->last_flush_latency_ns > (int64) 1000000)
		{
			pg_usleep(wal_group_commit_delay);

			/*
			 * After the adaptive delay, re-collect queued LSNs from any new
			 * waiters that arrived during our sleep.
			 */
			SpinLockAcquire(&groupCommitState->queue_lock);
			for (int i = 0; i < groupCommitState->num_waiters; i++)
			{
			...
			/*
			 * Re-check insertions to cover newly collected LSNs.  Safe to
			 * call because we know all insertions up to current insertpos
			 * are already done; we only move the pointer forward.
			 */
			insertpos = WaitXLogInsertionsToFinish(Max(insertpos, WriteRqstPtr));
		}
...

		{
			...
			INSTR_TIME_SUBTRACT(flush_end, flush_start);

			/* Record flush latency for adaptive batching decisions */
			if (groupCommitState != NULL)
				groupCommitState->last_flush_latency_ns =
					INSTR_TIME_GET_NANOSEC(flush_end);
		}
	...
	/*
	 * Wake up all group commit waiters.  Their LSNs should now be flushed
	 * since we advanced WriteRqstPtr to cover all of them.  Clear the queue
	 * and the flush_in_progress flag so the next leader can take over.
	 */
	if (groupCommitState != NULL)
	{
		int				num_to_wake;
		GroupCommitWaiter wake_list[GROUP_COMMIT_MAX_WAITERS];

		SpinLockAcquire(&groupCommitState->queue_lock);
		num_to_wake = groupCommitState->num_waiters;
		if (num_to_wake > 0)
			memcpy(wake_list, groupCommitState->waiters,
				   num_to_wake * sizeof(GroupCommitWaiter));
		groupCommitState->num_waiters = 0;
		SpinLockRelease(&groupCommitState->queue_lock);

		/* Clear flush_in_progress before waking, so new leaders can start */
		pg_atomic_write_u32(&groupCommitState->flush_in_progress, 0);

		/* Wake all waiters outside the spinlock */
		for (int i = 0; i < num_to_wake; i++)
		{
			PGPROC	   *proc = GetPGProcByNumber(wake_list[i].pgprocno);
...
/*
 * Return the data checksum algorithm version from the control file.
 * Returns 0 if checksums are disabled, 1 for FNV-1a, 2 for xxHash3.
 */
uint32
GetDataChecksumsVersion(void)
{
	Assert(ControlFile != NULL);
...
XLOGShmemInit(void)
{
	...
	if (foundCFile || foundXLog)
	{
		...
		/* Attach to existing group commit shared state */
		groupCommitState = (GroupCommitState *)
			ShmemInitStruct("WAL Group Commit",
							sizeof(GroupCommitState),
							&gcFound);
		Assert(gcFound);
	...
	/*
	 * Initialize WAL Group Commit shared state.
	 */
	{
		bool		found;
		...
		if (!found)
		{
			memset(groupCommitState, 0, sizeof(GroupCommitState));
			pg_atomic_init_u32(&groupCommitState->flush_in_progress, 0);
			SpinLockInit(&groupCommitState->queue_lock);
			groupCommitState->num_waiters = 0;
			groupCommitState->last_flush_latency_ns = 0;
		}
	}
}
